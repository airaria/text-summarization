...
class CopyNetWrapper(tf.nn.rnn_cell.RNNCell):
    ...
    def __init__(self, cell, encoder_states, encoder_input_ids, vocab_size,
        ...
        self.batch_size = tf.shape(self._encoder_input_ids)[0]
        self.src_len = tf.shape(self._encoder_input_ids)[1]
        self.scatter_shape = tf.stack([self.batch_size,vocab_size])
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        #copy_score = tf.Print(copy_score,[tf.shape(copy_score)])
        ...
        #prob_c_one_hot = tf.reduce_sum(
        #    encoder_input_mask * tf.expand_dims(copy_score,-1),axis=1)
        #prob_c_one_hot = tf.einsum("ijn,ij->in", encoder_input_mask, prob_c)

        batch_index = tf.tile(tf.range(self.batch_size)[:,None],[1,self.src_len])
        indices = tf.stack([batch_index, self._encoder_input_ids],-1)
        prob_c_one_hot = tf.scatter_nd(indices,copy_score,self.scatter_shape)
